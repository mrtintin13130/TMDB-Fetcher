...
document.getElementById("btn_movies_list").addEventListener("click", function () { toggleDiv("movies_list") }, event)
...
function toggleDiv(button) {
    ...
    movies_list = document.getElementById("movies_list")
    tv_by_title = document.getElementById("tv_shows_by_title")
    tv_by_id = document.getElementById("tv_shows_by_id")

    tabs = [home, movie_by_title, movie_by_id, movies_list, tv_by_title, tv_by_id]
    tabs.forEach(tab => {
